...
<head>
	...
	<script type="text/javascript">
		...
		{
			if(status==PAYMENT_STATUS_CAI_WU_YI_SHEN_HE && types==FALLBACK_TYPES_SUBMIT)
			{
				return '<a href="javascript:updateOcc('+id+');">汇款确认</a>&nbsp;&nbsp;<a href="javascript:fallBack('+id+')">回退</a>';
			}
			return "";
		}
	...
	<a:ajax 
		...
		urls="/finance/channelrebatereview/update_hr_pay_branch_channel_review_ajax" 
		pluginCode="110"
	/>
	<!--更新招生返款单回退状态-->
	<a:ajax 
				...
				<tr>
					...
					<td align="left">
						<select id="status" name="orderCeduChannel.status" class="txt_box_150">
							...
							<option selected="selected" value="<%=Constants.PAYMENT_STATUS_CAI_WU_YI_SHEN_HE %>">商务已审核</option>
