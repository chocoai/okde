...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			{
				refundAcademyCeduId = id;
				$('#message_confirm_one').dialog('open');
			}
			//修改状态：总部已汇款
			function updateStatusAjax()
			{
				status = REFUND_ADVANCE_STATUS_PAYMENT;
				ajax_2220_1();
			}
			//确认回调函数
		...
		<a:ajax 
			...
			pluginCode="1110" 
			urls="finance/refund/sum_refund_payment_academy_cedu_ajax"
		/>
		<!-- 修改状态 -->
		<a:ajax 
			parameters="{'refundAcademyCeduId':refundAcademyCeduId,'status':status}" 
			successCallbackFunctions="update_status_callback" 
			pluginCode="2220" 
			urls="finance/refund/update_refund_payment_academy_cedu_status_ajax"
		/>
	</head>
  ...
  <body>
	...
	<!-- 头开始 -->
		<head:head title="退费（院校垫付）">
			
		</head:head>
		<!--主体层开始 -->
		<body:body>
				<%@include file="_refund_academy_advance.jsp" %>
				<table class="gv_table_2">
			  		<tr>
					 	<th style="width:20px;"><img src="<ui:img url="/images/title_menu/title_left.gif" />" /></th>
					 	<th style="text-align:left; font-weight:bold;" class="feehtml">总部汇款院校</th>
				...
				<table class="add_table" border="0">
					<tr>
	             	...
	             	<tr >
	             		...
	             		<td>
	             			<select name="refundAcademyCedu.status" id="paymentStatus" class="txt_box_150">
								<option value="0">--请选择--</option>
								<option selected="selected" value="<%=Constants.REFUND_ADVANCE_STATUS_NOT_PAYMENT %>">总部未汇款</option>
								<option value="<%=Constants.REFUND_ADVANCE_STATUS_PAYMENT %>">总部已汇款</option>
								<option value="<%=Constants.REFUND_ADVANCE_STATUS_RECEIVE %>">院校已收款</option>					  
							</select>
						</td>
		                ...
		                <td align="left">
							...
							<input type="button" class="btn_black_61"  onclick="search001();" value="查询"/>
									...
									<page:plugin 
										...
										subStringLength="10"
										searchCountActionpath="finance/refund/count_refund_payment_academy_cedu_ajax"
										searchListActionpath="finance/refund/list_refund_payment_academy_cedu_ajax"
