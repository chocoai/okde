...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function showluqu(id,status)
			{
				...
				{
					return "<a href='javascript:stusenrolled("+id+");'>录取</a>";
				}
				else
				{
			...
			function ajax_updatestatus(data)
			{
				if(data.isfail)
				{
					jQuery("#showDialog").html('<b>未缴学费，缴完学费后才能录取！</b>');
					jQuery('#message_returns_tips').dialog("open");
				}
				else
				{
		...
		<a:ajax 
			...
			urls="finance/studentsenrolled/upate_student_senrolled_ajax"
		/>
		<!-- 批量录取-->
		<a:ajax 
			pluginCode="160"
	...
	<body>
		<!--头部层开始 -->
		<head:head title="学生录取">
			<html:a text="批量录取" icon="update" onclick="batchSenrolled()"/>
		</head:head>
		<!--主体层开始 -->
		<body:body>
				<form id="search_form">
							...
							<tr>
								...
								<td align="left">
									...
									<page:plugin 
										pluginCode="001"
										il8nName="crm"
										searchListActionpath="list_student_senrolled_page_ajax"
										searchCountActionpath="count_student_senrolled_page_ajax"
