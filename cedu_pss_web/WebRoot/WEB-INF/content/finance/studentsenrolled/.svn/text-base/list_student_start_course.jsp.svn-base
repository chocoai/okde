...
<html>
				...
				jQuery('#show_start_class').dialog({
					...
					buttons: {
						'确定': function() { 
							...
							isStartStatus=STU_IS_START_COURSE_TRUE;
							//alert(isStartStatus);
							ajax_160_1();//批量开课
							$(this).dialog("close"); 
						}, 
	...
	<body>
		<!--头部层开始 -->
		<head:head title="开课监控">
			<html:a text="批量开课" icon="update" onclick="batchApproval()"/>
		</head:head>
		<!--主体层开始 -->
		<body:body>
				<form id="search_form">
							...
							<tr>
								...
								<td align="left">
									<select class="txt_box_130" id="isStartCourse" name="student.isStartCourse">
										<option value="-1">--请选择--</option>
										<option selected="selected" value="<%=Constants.STU_IS_START_COURSE_FALSE %>">未开课</option>
										<option value="<%=Constants.STU_IS_START_COURSE_TRUE %>">已开课</option>
