...
<head>
	...
	<a:ajax 
		...
		urls="/finance/payacademycedu/del_academy_rebate_cedu_ajax" 
		pluginCode="110"
	/>
...
<body>
	<head:head title="院校返款单（总部）">
		<html:a text="添加返款单" icon="add" href="/finance/payacademycedu/add_academy_rebate" target="_blank"/>
	</head:head>
	<body:body>
	...
	<form id="searchForm">
		<table class="add_table">
			...
			<tr>	
				<td colspan="5"></td>
				<td><input type="button" onclick="showsearch()" class="btn_black_61" value="查询"/></td>
			</tr>
		</table>
	</form>
		...
		<page:plugin 
			...
			il8nName="finance"
			subStringLength="20"
			searchListActionpath="list_pay_academy_cedu_ajax"
			searchCountActionpath="count_pay_academy_cedu_ajax"
			columnsStr="#remittername;#amount;adjustPaied;#amountpaied;note;#remittancedate;#status;#types"
			customColumnValue="0,showtitle(remitterName);1,feePaymentValue(amount);2,feePaymentValue(adjustPaied);3,feePaymentValue(amountPaied);5,showtitle(remittanceDate);6,toStatus(status,types,id);7,showtypes(types)"
			pageSize="20"			
			view="http,/finance/payacademycedu/view_pay_academy_cedu,payAcademyCeduId,id,_blank"
