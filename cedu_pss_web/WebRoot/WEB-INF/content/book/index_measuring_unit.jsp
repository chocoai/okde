...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function delbc(id)
			{
				jQuery.post('<s:url value="delete_measuringunit"/>',{"id":id},
			      	  function(data)
			    		{
			    			if(data.results)
			    			{
			...
			function updmu()
			{
				...
				var code=$('#code').val();
				var name=$('#name').val();
				jQuery.post('<s:url value="update_measuringunit"/>',{"measuringunit.id":id,"measuringunit.name":name,"measuringunit.code":code},
			      	  function(data)
			    		{
			    			if(data.results)
			    			{
			...
			function yanzheng()
			{
			 var code=$("#measurcode").val();
			 var name=$("#measurname").val();
				if(code.trim()=="")
				{
				 alert("请输入编号");
				}else if(name.trim()=="")
		...
		{
			var image='<a href="javascript:delbc('+id+')">删除</a>  <a href="javascript:updatemu('+id+','+code+',\''+name+'\')">修改</a>';
			return image;
		}
		...
		function updatemu(id,code,name)
		{
			$('#id').val(id);
			$('#code').val(code);
			$('#name').val(name);
  ...
  <body>
		...
		<!--头部层开始 -->
		<head:head title="计量单位">
			<html:a text="新增" onclick="addmeasuringunit()" icon="add" />
		</head:head>
		<!--主体层开始 -->
		<body:body>
				<!--Search Begin-->
				...
				<div id="dataDiv1" style="display:block;">
					...
					<table class="gv_table_2">
						<tr>
							...
							<th style="text-align:left; font-weight:bold;">计量单位：</th>					
						</tr>
					</table>
				
					<page:plugin 
						pluginCode="123"
						il8nName="measuringunit"
						searchListActionpath="page_list_measuringunit"
						searchCountActionpath="page_count_measuringunit"
	...
	<div id="addDiv" style="display:none;">
		<form action="add_measuringunit" method="post" id="submits">
			<table class="add_table">  
				<tr>
					<td class="lable_100">编码：</td>
					<td>
						<input type="text" class="txt_box_130" name="measuringunit.code" id="measurcode"/>
					</td>
				</tr>
				<tr>
					<td class="lable_100">计算方式</td>
					<td>
					<input type="text" class="txt_box_130" name="measuringunit.name" id="measurname"/><br/>
					</td>
				</tr>
				<tr>
		...
		<form>
			<table class="add_table">  
				<tr>
					<td class="lable_100">编码：</td>
					<td>
						<input type="hidden" name="measuringunit.id" id="id" />
						<input type="text" class="txt_box_130" name="measuringunit.code" id="code"/>
					</td>
				</tr>
				<tr>
					<td class="lable_100">书商类型</td>
					<td>
					<input type="text" class="txt_box_130" name="measuringunit.name" id="name"/><br/>
