 	...
 	<script type="text/javascript" >
		...
		function findRelationships(){
			var order=$("#orderid").val();
				//通过表单实现js post提交
				document.write("<form action='find_cedubook_detail_center?orderCode="+order+"' method='post' name='findForm' style='display:none'>");
				document.write("</form>");
		...
		function insertRelationships(){
			...
			for(var i=1;i<10;i++)
			{
				...
				//通过表单实现js post提交
				document.write("<form action='add_invoice_detail_center?orderCode="+order+"&invoiceCode="+code+"&begintotal="+begintotal+"' method='post' name='lecturerForm' style='display:none'>");
				document.write("</form>");
		...
		<table class="add_table">
			<tr>
				<td align="left">订购单编号：<input name="" type="text" id="orderid" class="txt_box_100"   /></td>				
					<td align="left"><input name="" type="button" class="btn_black_61"  onclick="findRelationships()" value="查询" /></td>
			</tr>			
		</table>
		<div id="hidediv">
			<table class="add_table">
