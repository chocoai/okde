...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function delsm(id)
			{
				jQuery.post('<s:url value="delete_storagemode"/>',{"id":id},
			      	  function(data)
			    		{
			    			if(data.results)
			    			{
			...
			function updsm()
			{
				...
				var code=$('#code').val();
				var name=$('#name').val();
				jQuery.post('<s:url value="update_storagemode"/>',{"storagemode.id":id,"storagemode.name":name,"storagemode.code":code},
			      	  function(data)
			    		{
			    			if(data.results)
			    			{
		...
		{
			var image='<a href="javascript:delsm('+id+')">删除</a>  <a href="javascript:updatesm('+id+','+code+',\''+name+'\')">修改</a>';
			return image;
		}
		...
		function updatesm(id,code,name)
		{
			$('#id').val(id);
			$('#code').val(code);
			$('#name').val(name);
  ...
  <body> 
		...
		<!--头部层开始 -->
		<head:head title="入库方式">
			<html:a text="新增" onclick="addsm()" icon="add" />
		</head:head>
		<!--主体层开始 -->
		<body:body>
				<!--Search Begin-->
				...
				<div id="dataDiv1" style="display:block;">
					...
					<table class="gv_table_2">
						<tr>
							...
							<th style="text-align:left; font-weight:bold;">入库方式：</th>					
						</tr>
					</table>
				
					<page:plugin 
						pluginCode="123"
						il8nName="storagemode"
						searchListActionpath="page_list_storagemode"
						searchCountActionpath="page_count_storagemode"
	...
	<div id="addDiv" style="display:none;">
		<form action="add_storagemode" method="post" id="submits">
			<table class="add_table">  
				<tr>
					<td class="lable_100">编码：</td>
					<td>
						<input type="text" class="txt_box_130" name="storagemode.code" id="codes"/>
					</td>
				</tr>
				<tr>
					<td class="lable_100">入库方式</td>
					<td>
					<input type="text" class="txt_box_130" name="storagemode.name" id="names"/><br/>
					</td>
				</tr>
				<tr>
		...
		<form>
			<table class="add_table">  
				<tr>
					<td class="lable_100">编码：</td>
					<td>
						<input type="hidden" name="storagemode.id" id="id" />
						<input type="text" class="txt_box_130" name="storagemode.code" id="code"/>
					</td>
				</tr>
				<tr>
					<td class="lable_100">书商类型</td>
					<td>
					<input type="text" class="txt_box_130" name="storagemode.name" id="name"/><br/>
