...
function validate(){
	...
	$('#standardTable > tbody > tr').each(function(index){
		...
		if(! /^[1-9]\d*$/.test(floor) || !/^[1-9]\d*$/.test(ceil)){
			...
			$('.stdNo', this).css({color: 'red'});
			err.push('招生人数上限必须小于等于下限');
		}
		
		if(!value){
			err.push('金额/比例不能为空！');
		} else if(dealwithmoney(value)==-1){
			err.push('金额/比例必须为整数或小数！');
		} else {
...
jQuery(function(){
  ...
  <body>
		<!--头部层开始 -->
		<head:head title="招生返款政策标准 添加">
			<html:a text="关闭" icon="return" onclick="javascript:window.close();" target="_self"/>
		</head:head>
		<!--主体层开始 -->
		<body:body>

<form id="form" action="<uu:url url="/enrollment/chnlplcystd/save_chnl_plcy_std" />" onsubmit="return validate()" method="post" >
		...
		<table class="add_table"><tr><td align="center"><input class="btn_black_61" type="submit" value="添加" /></td></tr></table>
...
<table style="display:none" id="standardTemplate">
	<tr>
		...
		<td>
			<select class="stdValueForm" name="standards[1].rebatesId" onchange="chgUnit(this)">
		...
		<td align="right"><a href="javascript:void(0);" onclick="removeStd(this)">删除</a></td>
	</tr>
</table>
