...
<html>
	<head>
		...
		<a:ajax 
			...
			urls="enrollment/enrollmonitor/excel_export_jian_kong_student_admin"
		/>
		...
		<script type="text/javascript">
			function download(){
				...
				if(confirm("您确定要导出数据吗？")){
					jiankongstatus="";
					jQuery('[class=chkMonitorStatus]').each(function(){
						if(jQuery(this).attr("checked"))
						{
							if(jiankongstatus=="")
							{
								jiankongstatus+=this.value;
							}
							else
							{
								jiankongstatus+=","+this.value;
					...
					$("#jiankongzhuangtai").val(jiankongstatus);
					ajax_download_zip_1();
				}
			}
			var status='<%=Constants.STU_CALL_STATUS_YI_DAO_MING %>,<%=Constants.STU_CALL_STATUS_YI_JIAO_CE_SHI_FEI %>,<%=Constants.STU_CALL_STATUS_YI_CE_SHI %>,<%=Constants.STU_CALL_STATUS_JIAN_KONG_ZAI_SHENG_QIN %>,<%=Constants.STU_CALL_STATUS_JIAN_KONG_YI_CHENG_GONG %>,<%=Constants.STU_CALL_STATUS_JIAN_KONG_WEI_CHENG_GONG %>,<%=Constants.STU_CALL_STATUS_YI_FU_HE %>,<%=Constants.STU_CALL_STATUS_YI_LU_QU %>,<%=Constants.STU_CALL_STATUS_YI_JIAO_XUE_FEI %>';
			var jiankongstatus='<%=Constants.STU_MONITOR_STATUS_WEI_JIAN_KONG %>,<%=Constants.STU_MONITOR_STATUS_JIAN_KONG_ZAI_SHENG_QIN %>,<%=Constants.STU_MONITOR_STATUS_JIAN_KONG_YI_CHENG_GONG %>,<%=Constants.STU_MONITOR_STATUS_JIAN_KONG_WEI_CHENG_GONG %>';
			...
			function operating(id,monitorStatus){
				//if(monitorStatus==STU_MONITOR_STATUS_JIAN_KONG_YI_CHENG_GONG){
				//	return "--"
				//}else{
			...
			function monitorStatusValue(monitorStatus){
				//return monitorStatus=="<%=Constants.STU_MONITOR_STATUS_WEI_JIAN_KONG %>"?"未监控":"监控再申请";
	...
	<body>
		...
		<body:body>
				...
				<form id="search_form">
					<input type="hidden" name="student.monitorStatusIds" id="jiankongzhuangtai" value="<%=Constants.STU_MONITOR_STATUS_WEI_JIAN_KONG %>,<%=Constants.STU_MONITOR_STATUS_JIAN_KONG_ZAI_SHENG_QIN %>,<%=Constants.STU_MONITOR_STATUS_JIAN_KONG_YI_CHENG_GONG %>,<%=Constants.STU_MONITOR_STATUS_JIAN_KONG_WEI_CHENG_GONG %>"/>
					<input type="hidden" name="student.tuitionStatus" value="-1"/>
						...
						<tr>
							...
							<tr>
								...
								<td align="left" colspan="5">
									<input checked="checked" type="checkbox" value="<%=Constants.STU_MONITOR_STATUS_WEI_JIAN_KONG %>,<%=Constants.STU_MONITOR_STATUS_JIAN_KONG_ZAI_SHENG_QIN %>" class="chkMonitorStatus"/>未监控
									<input checked="checked" type="checkbox" value="<%=Constants.STU_MONITOR_STATUS_JIAN_KONG_YI_CHENG_GONG %>" class="chkMonitorStatus"/>已监控已成功
									<input checked="checked" type="checkbox" value="<%=Constants.STU_MONITOR_STATUS_JIAN_KONG_WEI_CHENG_GONG %>" class="chkMonitorStatus"/>已监控未成功
								</td>
							</tr>
							<tr>
								...
								<td align="left">
									<input  type="button" class="btn_black_61" onclick="searchForm()" value="查询"/>
									...
									<page:plugin 
										...
										columnsStr="branchName;name;certNo;enrollmentSourceName1;schoolName;academyenrollbatchName;levelName;majorName;monitorStatus;status;#public.operating"
										customColumnValue="8,monitorStatusValue(monitorStatus);9,statusValue(status);10,operating(id,monitorStatus)"
										searchFormId="search_form"
										view="http,/crm/view_home,studentId,id,_blank"
										params="'student.monitorStatus':-1,'student.statusIds':status,'student.gender':-1"
										isPackage="false"
									/>
