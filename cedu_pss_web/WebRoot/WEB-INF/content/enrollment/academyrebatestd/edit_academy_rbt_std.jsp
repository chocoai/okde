...
function cooRN() //重新校正返款标准 行 序号
{
	$('#rebate_table > tbody > tr').each(function(index){
		...
		$('.stdFloor', this).attr('name', 'standards['+index+'].floor');
		$('.stdCeil', this).attr('name', 'standards['+index+'].ceil');
		$('.stdValue', this).attr('name', 'standards['+index+'].value');
		$('.stdValueForm', this).attr('name', 'standards['+index+'].valueForm');
	});
}
	...
	$.ajax({
		...
		success: function(data){
			...
			for(var i=0; i<stdLen; i++){
				...
				$('.stdFloor', row).val(std.floor);
				$('.stdCeil', row).val(std.ceil);
				$('.stdValue', row).val(std.value);
				$('.ValueFormUnit', row).text(ValueForms[std.valueForm].unit);
				$('.stdValueForm', row).val(std.valueForm);
		...
		error: function(xhr){
			$.alert({msg:'加载原政策标准失败，请刷新页面',okText:'刷新',ok:function(){ window.location.reload(); }});
		}
	});
}
</script>
...
jQuery(function(){
  ...
  <body>
		<!--头部层开始 -->
		<head:head title="院校返款标准 修改">
			<html:a text="关闭" icon="return" onclick="self.close();" target="_self"/>
		</head:head>
		<!--主体层开始 -->
		<body:body>

<form id="form" action="<uu:url url="/enrollment/academyrebatestd/update_academy_rebate_policy" />" onsubmit="return validate()" method="post" >
		...
		<table class="add_table" border="0" cellpadding="2" cellspacing="2" id="rebate_table">
		  ...
		  <input type="hidden" value="${ param.policyId }" name="id" />
</form>
