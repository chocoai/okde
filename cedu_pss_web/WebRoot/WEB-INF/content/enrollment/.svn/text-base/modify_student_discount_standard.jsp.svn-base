...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function showassignment()
			{
				...
				//是否需要申请
				//if(${studentDiscount.isApplicationNeeded}==POLICY_APPLICATION_NEEDED_TRUE)
				//{
				//	jQuery("#isApplicationNeeded").attr("checked",true);
				//}
				//优惠审批（申请）
				var noisan=$("input[name='isApplicationNeeded']");
					noisan.each(function(i){
						if('${studentDiscount.isApplicationNeeded}'==$(this).val())
						{
				...
				//院校
				jQuery('#academyId').val(${studentDiscount.academyId});	
				//费用科目
				jQuery('#feesubjectId').val(${studentDiscount.feeSubjectId});
				//初始化缴费次数
				
				if(${feeSucject.isMultiplePayment}==IS_MULTIPLE_PAYMENT_TRUE)
				{
					$('#feePaymentId').empty();
				...
				//缴费次数
				jQuery('#feePaymentId').val(${studentDiscount.feePaymentId});
				//优惠方式
				jQuery('#discountWayId').val(${studentDiscount.discountWayId});
				//优惠主体
				jQuery('#targetObjectId').val(${studentDiscount.targetObjectId});
				
				//是否共同承担
				jQuery('#isShared').val(${studentDiscount.isShared});
				jQuery('#targetObjectId').change();
  ...
  <body>
<!-- 头开始 -->
		<head:head title="修改学生优惠政策标准">
			<html:a text="返回" icon="return" onclick="history.go(-1);"/>
		</head:head>
		<!--主体层开始 -->
		<body:body>
				<form id="myform" method="post" action="modify_student_discount_standard" onsubmit="return showsubmit();">
				 ...
				 <table class="gv_table_2">
				  ...
				  <table class="add_table" id="enrollbatch" class="enrollbatch" border="0" cellpadding="2" cellspacing="2">
				  	<tbody>
						...
						<tr>
							...
							<td >
					  			<input type="text" name="studentDiscount.title" id="title" value="${studentDiscount.title}" class="txt_box_150"/>
							</td>
						</tr>
				  	</tbody>
				  </table>
					 ...
					 <tbody>
						...
						<tr>
					  		...
					  		<td align="left">
								...
								<input type="radio" name="isApplicationNeeded" id="isApplicationNeeded" value="<%=Constants.STUDENT_DISCOUNT_AUDIT_CEDU%>"/>总部审批
					  	...
					  	<tr>
					  		...
					  		<td align="left">
					  			起始时间：<input type="text" value="<s:date name='studentDiscount.activityBeginDate' format='yyyy-MM-dd'/>" name="studentDiscount.activityBeginDate" id="activityBeginDate" class="Wdate" onfocus="WdatePicker({skin:'whyGreen',maxDate:'#F{$dp.$D(\'activityEndDate\',{d:-1});}'})" readonly="readonly"/>
					  			~~结束时间：<input type="text" value="<s:date name='studentDiscount.activityEndDate' format='yyyy-MM-dd'/>" name="studentDiscount.activityEndDate" id="activityEndDate" class="Wdate" onfocus="WdatePicker({skin:'whyGreen',minDate:'#F{$dp.$D(\'activityBeginDate\',{d:1});}'})" readonly="readonly"/>
						...
						<tr>
					  		...
					  		<td align="left">
					  			起始时间：<input type="text" value="<s:date name='studentDiscount.useBeginDate' format='yyyy-MM-dd'/>" name="studentDiscount.useBeginDate" id="useBeginDate" class="Wdate" onfocus="WdatePicker({skin:'whyGreen',maxDate:'#F{$dp.$D(\'useEndDate\',{d:-1});}'})" readonly="readonly"/>
					  			~~结束时间：<input type="text" value="<s:date name='studentDiscount.useEndDate' format='yyyy-MM-dd'/>" name="studentDiscount.useEndDate" id="useEndDate" class="Wdate" onfocus="WdatePicker({skin:'whyGreen',minDate:'#F{$dp.$D(\'useBeginDate\',{d:1});}'})" readonly="readonly"/>
							</td>
						...
						<tr>
					  		...
					  		<td align="left">
					  			<input type="text" value="${studentDiscount.money}" name="studentDiscount.money" id="money" class="txt_box_150"/><span id="rebates_way_unit" class="rebates_way_unit" style="color: black !important;">%</span>
								&nbsp;&nbsp;&nbsp;
								<span id="hid1" class="hid1" style="display:none"></span>
								<span id="hid1" class="hid1" style="display:none"><span id="rebates_way_name1"></span><input type="text" value="${studentDiscount.delta}" name="studentDiscount.delta" id="delta" class="txt_box_150"/><span id="rebates_way_unit1" class="rebates_way_unit" style="color: black !important;"></span></span>
							</td>
				  ...
				  <table class="add_table"><tr><td align="center"><input class="btn_black_61" type="submit" value="修改" /></td></tr></table>
				</form>
			</body:body>
