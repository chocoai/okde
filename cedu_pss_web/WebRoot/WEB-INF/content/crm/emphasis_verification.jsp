...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function changeTab(isEmphasisVerification){
				$("#isEmphasisVerification").val(isEmphasisVerification);
				search001();
			}
			...
			function ajax_stustatusajax(data)
			{
			    ...
			    if(data.stuStatusList!=null && data.stuStatusList.length>0)
			    {
			    	$.each(data.stuStatusList,function(){	
			    		//if(this.id==STU_CALL_STATUS_YI_JIAO_CE_SHI_FEI){
			    		if(false){
		...
		<a:ajax 
			...
			urls="enrollment/enrollmonitor/excel_export_repeat_student"
		/>
	</head>
	<body>
		<!--头部层开始 -->
		<head:head title="重点监控">
		</head:head>
		<!--主体层开始 -->
		<body:body>
				<div>
					<ul id="menu">
						<li><a href="#" onclick="changeTab(<%=StudentConstants.IS_EMPHASIS_VERIFICATION_MOBILE %>);" class="current">学生手机号重复</a></li>
						<li><a href="#" onclick="changeTab(<%=StudentConstants.IS_EMPHASIS_VERIFICATION_BRANCH_STUDENT_NAME %>);" >学习中心下姓名重复</a></li>
					</ul>
				</div>	
				<form id="search_form">
					<input name="student.isEmphasisVerification" id="isEmphasisVerification" value="<%=StudentConstants.IS_EMPHASIS_VERIFICATION_MOBILE %>" type="hidden"/>
							...
							<tr>
								...
								<td align="left">
									...
									<page:plugin 
										...
										columnsStr="name;certNo;mobile;status;gender;enrollmentSourceName1;branchName;schoolName;academyenrollbatchName;levelName;majorName;#public.operating"
										customColumnValue="3,statusValue(status);4,sexValue(gender);11,operating(id)"
										pageSize="10"
