...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function wayAndSourceCallback(doc){
								...
								$(doc.enrollmentSources).each(function(){
									...
									//添加老生续读选项by dongminghao 2012-03-28 下午17:26:41
		...
		<a:ajax pluginCode="003" urls="/crm/update_student_info_ajax" parameters="$('#create_student_form').serializeObject()" successCallbackFunctions="updateStudentCallBack" />
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
						...
						if($("#student_id").val()=="0"){
							...
							if($("#source").val()=="0"){
								...
								if($("#way").val()=="0"){
									...
									//为社招或者老生续读 合作方可以为空
									if($('#source').val()==WEB_STU_SOURCE_DEFAULT || $('#source').val()==WEB_STU_LAO_SHENG_XU_DU){
										//社招
										updateStudent();
									}else{
				...
				$("#sfzh").formValidator({onShow:"请输入证件号",onFocus:"请输入证件号",onCorrect:"输入正确"}).inputValidator({min:1,max:18,onError:"你输入的号码错误,请确认"})
				.ajaxValidator({
					dataType : "json",
