...
<html>
	<head>
		...
		<script type="text/javascript">
			$(document).ready(function()
				...
				//操作学生
				function customStudent(id,status)
				{
					//return "<a href='#' onclick='deleteStudent("+id+","+status+")'>删除</a>";
					return "<a href='javascript:deleteStudent("+id+","+status+")'>删除</a>";
				}
				//删除学生
				function deleteStudent(id,status){
					//如果该学生为已报名之后的状态则给出提示
					if(status>=STU_CALL_STATUS_YI_JIAO_CE_SHI_FEI&&status!=STU_CALL_STATUS_WU_YI_YUAN_CHUSHI)
					{
						$('#messageDiv').html('该学生为'+status.getStudentStatus()+'<br>确认执行删除？');
					}
					else
					{
						$('#messageDiv').html('确认执行删除？');
					}
					student_id=id;
				...
				function delete_student_callback(data){
					...
					if(data.errorCode=="200001"){
						...
						alert("该学生不存在或者已删除！");
						refresh001();//刷新列表
					}else if(data.errorCode=="200003"){//缴费单存在未作废
						alert("该学生存在没有作废的缴费单！");
					}else if(data.errorCode=="200004"){//老生续读关联的新生
						alert("该学生为老生续读关联的新生！");
					}
					
				}
			</script>
			<a:ajax parameters="{'student.id':student_id}" successCallbackFunctions="delete_student_callback" pluginCode="0001" urls="/crm/crm_delete_student_all"/>
			<!-- 学生共有的查询条件-->
			<a:ajax 
				pluginCode="110"
	...
	<body>
		<!--头部层开始 -->
		<head:head title="学生删除">
		</head:head>
		<!--主体层开始 -->
		<body:body>
				<div id="message_confirm" style="display:none">
						<div id="messageDiv">确认执行该操作！</div>
				</div>
				<form id="search_form">
							...
							<tr>
								...
								<td align="left">
									...
									<page:plugin 
										...
										columnsStr="name;certNo;status;gender;branchName;schoolName;academyenrollbatchName;levelName;majorName;#public.operating"
										customColumnValue="2,statusValue(status);3,sexValue(gender);9,customStudent(id,status)"
