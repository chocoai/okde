...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function ajax_push_userajax(data)
			{
				if(data!=null && data.users!=null)
				{
					...
					$('#creatorId').html(lists);
				}
			}
			function download(){
				if(confirm("您确定要导出数据吗？")){
					if($("#table002").css("display")!="none")
					{ajax_download_zip2_1();}
					else if($("#table003").css("display")!="none")
					{ajax_download_zip_1();}
		...
		<a:ajax 
			...
			urls="/crm/export/excel_export_cc_student1"
		/>
		<!-- 下载地址 -->
		<a:ajax 
			parameters="jQuery('#search_form').serializeObject()" 
			successCallbackFunctions="excel_export_callback" 
			pluginCode="download_zip2" 
			urls="/crm/export/excel_export_cc_student2"
		/>
		...
		<script type="text/javascript">
			...
			function studentDataSourceValue(studentDataSource){
				return studentDataSource.getStudentDataSource();
			}
			//根据状态查询学生跟进列表
			...
			function wuYiYuanCallBack002(data){
				if(data.result.recordCount>0){
					$("#export_student_data_btn").css({"display":""});
				}else{
					$("#export_student_data_btn").css({"display":"none"});
					...
					<table class="add_table" cellpadding="2" cellspacing="2" border="0">
						...
						<tr>
							...
							<tr>
								...
								<td align="left">
									...
									<input id="starttime" class="Wdate" type="text"
										name="student.createStartDate" value=""
								...
								<td align="left">
									...
									<input id="endtime" class="Wdate" type="text"
										name="student.createEndDate" value="" size="17"
								...
								<td align="left">
									<div class="date_class">
									<select id="creatorId" name="student.pushId" class="txt_box_130">
										<option value="0">--请选择--</option>
									</select>
									</div>
								</td>
							...
							<tr>
								...
								<td align="left">
									<select id="studentDataSourceIds" name="student.studentDataSourceIds"  class="txt_box_130">
										<option value="<%=Constants.STU_SOURCE_CC %>,<%=Constants.STU_SOURCE_NP %>">--请选择--</option>
								...
								<td align="left">
									...
									<page:plugin 
										...
										columnsStr="name;mobile;status;branchName;gender;studentDataSource;enrollmentSourceName1;enrollmentWayName;schoolName;academyenrollbatchName;levelName;majorName"
										customColumnValue="2,statusValue(status);4,sexValue(gender);5,studentDataSourceValue(studentDataSource)"
										pageSize="10"
										...
										params="'student.callStatus':-1,'student.status':STU_CALL_STATUS_YU_BAO_MING"
									/>
									<page:plugin 
										isonLoad="false"
										pluginCode="002"
										...
										update="http,/crm/call_home,studentId,id,_blank"
										searchFormId="search_form"
										params="'student.callStatus':-1,'student.status':STU_CALL_STATUS_WU_YI_YUAN_CHUSHI"
										countCallback="wuYiYuanCallBack"
									/>
									<page:plugin 
										pluginCode="003"
										isonLoad="false"
										...
										searchListActionpath="crm_student_list"
										searchCountActionpath="crm_student_count"
										columnsStr="name;mobile;status;branchName;gender;studentDataSource;enrollmentSourceName1;enrollmentWayName;schoolName;academyenrollbatchName;globalBatchName;#createDate;pushName;pushDate;followUpName;latestFollowUpDate"
										customColumnValue="2,statusValue(status);4,sexValue(gender);5,studentDataSourceValue(studentDataSource);11,createValue(createDate)"
										pageSize="10"
										searchFormId="search_form"
										view="http,/crm/view_home,studentId,id,_blank"
										params="'student.callStatus':-1,'student.startStatusId':STU_CALL_STATUS_YU_BAO_MING,'student.endStatusId':STU_CALL_STATUS_WU_YI_YUAN_CHUSHI"
