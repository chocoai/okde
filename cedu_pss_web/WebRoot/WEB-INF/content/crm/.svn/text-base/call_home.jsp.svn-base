...
<html>
	<head>
		...
		<!--招生批次(院校、全局批次)-->
		<a:ajax successCallbackFunctions="ajax_batch" parameters="{academyId:jQuery('#academyId').val(),globalBatchId:jQuery('#globalBatchId').val()}" urls="/enrollment/cascade_global_batch_academy_qiyong_batch_ajax" pluginCode="140"/>
		...
		<script type="text/javascript">
			...
			function select_student_phone_success_callback(doc)
			{
				if(null!=doc.student)
				{
					...
					if(null!=doc.student.globalBatch)
					{
						$('#globalBatchId').val(doc.student.globalBatch);
					}
		...
		<a:ajax pluginCode="003" urls="/crm/update_student_cc" parameters="$('#create_student_form').serializeObject()" successCallbackFunctions="updateStudentCallBack" />
		
		<a:ajax pluginCode="004" urls="/crm/student_way_list"  successCallbackFunctions="wayAndSourceCallback"  isOnload="all"/>
		
		<a:ajax pluginCode="005" urls="crm/student_channel_list" parameters="{'id':parseInt($('#source').val()),'branchId':studentBranchId}" successCallbackFunctions="channelCallback" />
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				//证件号
				$("#sfzh").formValidator({empty:true,onShow:"请输入证件号",onFocus:"请输入证件号",onCorrect:"输入正确"}).functionValidator({fun:isCardID})
				.ajaxValidator({
					dataType : "json",
	...
	<body>
		...
		<body:body>
						...
						<tr>
							<td style="width: 44%;" valign="top">
									...
									<page:plugin 
										...
										params="'followUp.studentId':(studentId==0?-2:studentId),'result.order':'createdTime','result.sort':'desc'"
									/>
							...
							<form id="create_student_form" style="margin:0px;">
								...
								<table class="add_table" border="0">
									...
									<tr>
										...
										<td align="left">
											<textarea id="remark" rows="4" maxlength="400" style="overflow:hidden" class="txt_box_350" name="student.remark">${student.remark}</textarea>
										</td>
									</tr>
									...
									<tr>
										...
										<td align="left" id="hezuofang_t">
												<select id="channel" name="student.channelId"  class="txt_box_150"></select>
										</td>
					...
					<c:if test="${student!=null&&(student.status==1||student.status==22)}">
								<table class="add_table">
									<tr>
										<td align="right" width="100px;" >
											<span>*</span>学习中心：
								...
								<table class="add_table">
									...
									<tr>
										<td align="center" colspan="2">
											<c:if test="${student!=null}">
												<input  class="btn_black_61" type="submit" id="save" onclick="changeHiddenStatus(${student.status});"  value="保存" />
											</c:if>
											<c:if test="${student==null}">
