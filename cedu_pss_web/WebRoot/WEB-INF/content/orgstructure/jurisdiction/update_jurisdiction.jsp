...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			{
				if(data.result) alert_dialog("保存成功");
				else alert_dialog("保存失败");
				
			}
		</script>
		<a:ajax successCallbackFunctions="saveGroupUser_callback" pluginCode="004" traditional="true" parameters="$('#up_form').serializeObject()" urls="/orgstructure/jurisdiction/save_jurisdiction"/>
		...
		{
			$("input[name='deplist']").each(function()
			{
  ...
  <body>
		...
		<body:body>
				...
				<tr>
					...
					<s:iterator value="bdlist" var="branchDepartment">
						<s:if test='#ucount==1'>
							</tr><tr>
							<s:set id="ucount" value="0" />
						</s:if>
						<td>${branchDepartment.branch.name }</td>
						<td>
							<s:iterator value="departments" var="department">
							<input type="checkbox" id="u${id}" name="deplist" value="${id}"/>${department.name }
							&nbsp;&nbsp;
							</s:iterator>
						</td>
						<s:set id="ucount" value="#ucount+1" />
					</s:iterator>
					<s:iterator value="haslist" var="s">
						
						<script type="text/javascript">
							$("#u"+${s }).attr("checked",true);
						</script>
					</s:iterator>	
				</tr>
				<tr>
