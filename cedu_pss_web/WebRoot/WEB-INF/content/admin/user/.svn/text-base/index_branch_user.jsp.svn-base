...
<html>
			...
			function modifystatus_callback(data){
				if(data.results){
			    	...
			    	searchpage();
			    	 ...
			    	 //show('showDialog','该用户存在招生指标和学生，无法停用!',150,100);	
  ...
  <body>
		...
		<!--头部层开始 -->
		<head:head title="${branchName }用户管理">
		...
		<body:body>
				...
				<div id="dataDiv1" style="display:block;">
					...
					<page:plugin 
						...
						customColumnValue="0,showphoto(photoUrl);6,departmentName(department);7,jobValue(job);8,changestatus(status,id)"
						pageSize="10"
