...
public class JsonAddChannelRebateSpecialReviewAction extends BaseAction
{
	 ...
	 * @throws Exception
	 */
	@Action(value = "add_channel_rebate_special_review_ajax", results = { @Result(name = "success", type = "json", params = {
				...
				if(newNeedFpdIds!=null && !newNeedFpdIds.equals(""))
				{
					...
					for(int i=0;i<fpdIds.length;i++)
					{
						...
						if(feeObj!=null)
						{
							...
							feeObj.setChannelRebateTimeId(orderCeduChannel.getChannelRebateTimeId());
							fpdlist.add(feeObj);
