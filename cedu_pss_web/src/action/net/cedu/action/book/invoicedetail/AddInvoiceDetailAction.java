 ...
 * @修改时间：
 */
public class AddInvoiceDetailAction extends BaseAction {
	...
	// 先查询出订购单明细
	@Action(value = "find_cedubook_detail_sel", results = { @Result(name = "success", location = "add_invoice_detail.jsp"),
														@Result(name = "input", location = "add_invoice_detail.jsp")})
	public String find() {
		try {
			//查询订购单
			...
			if (null != ceduBookOrder) {
				...
				if(ceduBookOrder.getBranchId()!=0)
				{
	...
	// 增加
	@Action(value = "add_invoice_detail", results = {
