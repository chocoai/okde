...
/**
 * 院校返款单新增(院校返款单新增(现需返款的缴费单明细ajax)
 * 
 * @author lixiaojun
 * 
 */
@ParentPackage("json-default")
public class JsonAddPayAcademyCeduFeePaymentAction extends BaseAction
{
	// 分页结果
	 ...
	 * @throws Exception
	 */
	@Action(value = "count_academy_rebate_feepaymentdetail_for_now_ajax", results = { @Result(name = "success", type = "json", params = {
	 ...
	 * @throws Exception
	 */
	@Action(value = "list_academy_rebate_feepaymentdetail_for_now_ajax", results = { @Result(name = "success", type = "json", params = {
			"contentType", "text/json" }) })
	 ...
	 * @throws Exception
	 */
	@Action(value = "test_feepaymentdetail_for_academy_rebate_ajax", results = { @Result(name = "success", type = "json", params = {
			"contentType", "text/json" }) })
	...
	{	
		...
		if(addFeepdIds!=null && addFeepdIds.length()>0)
		{
			...
			for (int i = 0; i < ids.length; i++)
			{
				...
				{
					AcademyRebatePolicyDetailStandard arpds=this.arpdsBiz.findByStudentIdFeeSubjectIdForAcademyRebate(this.studentBiz.findStudentById(this.feePaymentDetailBiz.findById(Integer.parseInt(ids[i])).getStudentId()), this.feePaymentDetailBiz.findById(Integer.parseInt(ids[i])).getFeeSubjectId());
					if(arpds==null)
					{
						FeePaymentDetail fpd=this.feePaymentDetailBiz.findById(Integer.parseInt(ids[i]));
						if(fpd!=null)
					...
					else
					{
						...
						//统计返款总金额
						allRebateMoney=(new BigDecimal(arpdsBiz.findAcademyRebateByfeePaymentDetailId(Integer.valueOf(ids[i]))).add(new BigDecimal(allRebateMoney))).doubleValue();
					}
				}
			}
		}
	 ...
	 * @throws Exception
	 */
	@Action(value = "del_feepaymentdetail_for_academy_rebate_ajax", results = { @Result(name = "success", type = "json", params = {
			"contentType", "text/json" }) })
	public String delfpdforacademyrebate() throws Exception 
	{
		if(allFeepdIds!=null && !allFeepdIds.equals("") && delFeepdIds!=null && !delFeepdIds.equals(""))
		{
			...
			for(int i=0;i<allIds.length;i++)
			{
				...
				{
					adrmoney=(new BigDecimal(arpdsBiz.findAcademyRebateByfeePaymentDetailId(Integer.valueOf(allIds[i]))).add(new BigDecimal(adrmoney))).doubleValue();
				}
			}
