...
public class JsonAddPayBranchChannelSpecialAction extends BaseAction
{
	 ...
	 * @throws Exception
	 */
	@Action(value = "add_pay_branch_channel_special_ajax", results = { @Result(name = "success", type = "json", params = {
		...
		if(orderCeduChannel!=null && feepdIds!=null && feepdIds.length()>0)
		{
			...
			if(feepdIds!=null && !feepdIds.equals(""))
			{
				...
				for(int i=0;i<fpdIds.length;i++)
				{
					...
					if(feeObj!=null)
					{
						feeObj.setPaymentByChannel(this.cpdsBiz.findChannelRebateSpecialByfeePaymentDetailIdChannelIdPolicyStatus(feeObj.getId(),orderCeduChannel.getChannelId(),orderCeduChannel.getMinorChannelId(),orderCeduChannel.getPoliceStatus()));
