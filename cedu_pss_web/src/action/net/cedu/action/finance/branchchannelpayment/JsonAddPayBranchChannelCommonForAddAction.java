...
/**
 * 招生返款单新增(招生返款单新增(现需返款的缴费单明细ajax)通用返款政策
 * 
 * @author lixiaojun
 * 
 */
@ParentPackage("json-default")
public class JsonAddPayBranchChannelCommonForAddAction extends BaseAction
{
	// 分页结果
	 ...
	 * @throws Exception
	 */
	@Action(value = "count_channel_rebate_common_feepaymentdetail_for_now_ajax", results = { @Result(name = "success", type = "json", params = {
	 ...
	 * @throws Exception
	 */
	@Action(value = "list_channel_rebate_common_feepaymentdetail_for_now_ajax", results = { @Result(name = "success", type = "json", params = {
			"contentType", "text/json" }) })
	public String channelForNowList() throws Exception 
	{	
		result.setList(this.feePaymentDetailBiz.findFeePaymentDetailListByPageForNowAddChannelRebateCommon(feepdIds,result));
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "test_feepaymentdetail_for_channel_rebate_common_ajax", results = { @Result(name = "success", type = "json", params = {
			"contentType", "text/json" }) })
	...
	{	
		...
		if(addFeepdIds!=null && addFeepdIds.length()>0)
		{
			...
			for (int i = 0; i < ids.length; i++)
			{
				...
				if(oldFeepdIds!=null && !oldFeepdIds.equals(""))
				{
					...
					if(zuhefpd.indexOf(","+ids[i]+",")!=-1)
					{
						...
						if(fpd!=null)
						{
							...
							//费用科目名称
							FeeSubject fb=this.feeSubjectBiz.findFeeSubjectById(fpd.getFeeSubjectId());
							if(fb!=null)
							{
								fpd.setPaymentSubjectName(fb.getName());
							}
							else
							{
				...
				{
					Student stud=this.studentBiz.findStudentById(this.feePaymentDetailBiz.findById(Integer.parseInt(ids[i])).getStudentId());
					if(stud!=null)
					{
						//为了匹配通用政策
						stud.setBranchId(-1);
					...
					ChannelPolicyDetailStandard cpds=this.cpdsBiz.findByStudentFeeSubjectIdForChannelRebate(stud, this.feePaymentDetailBiz.findById(Integer.parseInt(ids[i])).getFeeSubjectId());
					if(cpds==null)
					{
						ishaspol=false;
						return SUCCESS;
					...
					else
					{
						...
						//统计返款总金额
						allRebateMoney=(new BigDecimal(cpdsBiz.findChannelRebateCommonByfeePaymentDetailId(Integer.valueOf(ids[i]))).add(new BigDecimal(allRebateMoney))).doubleValue();
					}
				}
			}
		}
	 ...
	 * @throws Exception
	 */
	@Action(value = "del_feepaymentdetail_for_channel_rebate_common_ajax", results = { @Result(name = "success", type = "json", params = {
			"contentType", "text/json" }) })
	public String delfpdforacademyrebate() throws Exception 
	{
		if(allFeepdIds!=null && !allFeepdIds.equals("") && delFeepdIds!=null && !delFeepdIds.equals(""))
		{
			...
			for(int i=0;i<allIds.length;i++)
			{
				...
				{
					adrmoney=(new BigDecimal(cpdsBiz.findChannelRebateCommonByfeePaymentDetailId(Integer.valueOf(allIds[i]))).add(new BigDecimal(adrmoney))).doubleValue();
				}
			}
	...
	public boolean isIshaspol() {
		return ishaspol;
	}

	public void setIshaspol(boolean ishaspol) {
		this.ishaspol = ishaspol;
	}
