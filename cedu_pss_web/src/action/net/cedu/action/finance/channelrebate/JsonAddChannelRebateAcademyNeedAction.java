...
public class JsonAddChannelRebateAcademyNeedAction extends BaseAction
{
	// 分页结果
	 ...
	 * @throws Exception
	 */
	@Action(value = "count_channel_rebate_fpd_academy_for_now_review_ajax", results = { @Result(name = "success", type = "json", params = {
...
//			result.setRecordCount(nStr.size());
	 ...
	 * @throws Exception
	 */
	@Action(value = "list_channel_rebate_fpd_academy_for_now_review_ajax", results = { @Result(name = "success", type = "json", params = {
			"contentType", "text/json" }) })
	 ...
	 * @throws Exception
	 */
	@Action(value = "test_fpd_for_channel_rebate_academy_review_ajax", results = { @Result(name = "success", type = "json", params = {
			"contentType", "text/json" }) })
	...
	{	
			...
			for (int i = 0; i < ids.length; i++)
			{
				...
				if(fpd!=null && fpd.getFeeSubjectId()>0)
				{
					...
					if(student!=null)
					{
						...
						{
							cpds=this.cpdsBiz.findCpdsByStudentFeeSubjectIdForChannelRebateReview(student, fpd.getFeeSubjectId());
						}
						if(cpds==null)
						{
							//缴费单的编号
							...
							//判断是不是有没有政策
							//匹配政策
							ChannelPolicyDetail cpd=this.channelPolicyDetailBiz.findByStudent(student,fpd.getFeeSubjectId());
							if(cpd==null)
							{
		...
		if(addnewfpdIds!=null && !addnewfpdIds.equals(""))
		{
			this.cpdsBiz.updateStuFpdChannelRebateByNewFpdIdsOldFpdIdsReview(addnewfpdIds,newFeepdIds);
		}
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "del_fpd_for_channel_rebate_academy_review_ajax", results = { @Result(name = "success", type = "json", params = {
			"contentType", "text/json" }) })
	public String delfpdforacademyrebate() throws Exception 
	{
		if(allFeepdIds!=null && !allFeepdIds.equals("") && delFeepdIds!=null && !delFeepdIds.equals(""))
		{
			...
			this.cpdsBiz.updateStuFpdChannelRebateByDelFpdIdsOldFpdIdsReview(delFeepdIds, newdelFeepdIds);
		}
		else
		{
			isfail=false;
	 ...
	 * @throws Exception
	 */
	@Action(value = "count_channel_rebate_fpd_academy_for_now_all_money_ajax", results = { @Result(name = "success", type = "json", params = {
			"contentType", "text/json" }) })
