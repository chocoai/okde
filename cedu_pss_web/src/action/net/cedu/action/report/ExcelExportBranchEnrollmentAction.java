 ...
 *
 */
public class ExcelExportBranchEnrollmentAction extends BaseAction {
	...
	public String execute() throws Exception {
		...
		Map map=branchEnrollmentReport.statistics(mapParams,dateParams);
		List branchList = (List)map.get("branchList");
		if (branchList != null && branchList.size() != 0) {
			downLoadFile(createExcel(branchList, "学习中心项目明细表",map), "学习中心项目明细表"+ ".xls");
	...
	public HSSFWorkbook createExcel(List branchList, String title,Map resultMap)
		...
		try {
			...
			createCell(wb, head, 1, "学校",HSSFColor.GREY_25_PERCENT.index);
			...
			for (int i = 0; i < branchList.size(); i++) {
				...
				//学习中心集合
				List academyList=getList(branch.get("academy_branch_list"));
				if(academyList!=null&&academyList.size()!=0){
					for (int j = 0; j < academyList.size(); j++) {
						...
						createCell(wb, bodyRow, 1, academy.get("academy_name").toString(),HSSFColor.WHITE.index);
						
						createCell(wb, bodyRow, 2, academy.get("academy_zhi_biao").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 3, academy.get("branch_bao_ming_count").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 4, academy.get("branch_bao_ming_count_p").toString(),HSSFColor.WHITE.index);
						
						createCell(wb, bodyRow, 5, academy.get("branch_lu_qu_count").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 6, academy.get("branch_no_lu_qu_count").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 7, academy.get("branch_lu_qu_count_p").toString(),HSSFColor.WHITE.index);
						
						createCell(wb, bodyRow, 8, academy.get("branch_jiao_fei_count").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 9, academy.get("branch_jiao_fei_jin_e").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 10, academy.get("branch_jiao_fei_count_p").toString(),HSSFColor.WHITE.index);
					...
					createCell(wb, bodyRow, 2, branch.get("academy_branch_zhi_biao").toString(),HSSFColor.SKY_BLUE.index);
					createCell(wb, bodyRow, 3, branch.get("academy_branch_bao_ming_count").toString(),HSSFColor.SKY_BLUE.index);
					createCell(wb, bodyRow, 4, branch.get("academy_branch_bao_ming_count_p").toString(),HSSFColor.SKY_BLUE.index);
					
					createCell(wb, bodyRow, 5, branch.get("academy_branch_lu_qu_count").toString(),HSSFColor.SKY_BLUE.index);
					createCell(wb, bodyRow, 6, branch.get("academy_branch_no_lu_qu_count").toString(),HSSFColor.SKY_BLUE.index);
					createCell(wb, bodyRow, 7, branch.get("academy_branch_lu_qu_count_p").toString(),HSSFColor.SKY_BLUE.index);
					
					createCell(wb, bodyRow, 8, branch.get("academy_branch_jiao_fei_count").toString(),HSSFColor.SKY_BLUE.index);
					createCell(wb, bodyRow, 9, branch.get("academy_branch_jiao_fei_jin_e").toString(),HSSFColor.SKY_BLUE.index);
					createCell(wb, bodyRow, 10, branch.get("academy_branch_jiao_fei_count_p").toString(),HSSFColor.SKY_BLUE.index);
					...
					//没有授权的学习中心
					HSSFRow bodyRow = sheet.createRow(row++);
					createCell(wb, bodyRow, 0, branch.get("branch_name").toString(),HSSFColor.RED.index);
					createCell(wb, bodyRow, 1, "该中心下没有院校!",HSSFColor.RED.index);
