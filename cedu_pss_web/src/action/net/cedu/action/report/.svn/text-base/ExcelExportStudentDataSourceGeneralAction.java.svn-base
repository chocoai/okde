...
public class ExcelExportStudentDataSourceGeneralAction extends BaseAction {
	...
	public String execute() throws Exception {
		...
		if (reportList != null && reportList.size() != 0) {
			downLoadFile(createExcel(reportList, "学生数据来源统计报表(总)",map), "学生数据来源统计报表(总)"+ ".xls");
			return null;
		}
		return SUCCESS;
	...
	public HSSFWorkbook createExcel(List reportList, String title,Map result)
		...
		try {
			...
			for (int i = 0; i < reportList.size(); i++) {
				...
				for (int j = 0; j < xuexizhongxinList.size(); j++) {
					...
					for (int k = 0; k < fuwuzhanList.size(); k++) {
						...
						createCell(wb, bodyRow, 5, fuwuzhanHeJiMap.get("wangluobaomingCountSum").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 6, fuwuzhanHeJiMap.get("wangluobaomingCountPSum").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 7, fuwuzhanHeJiMap.get("hujiaozhongxingCountSum").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 8, fuwuzhanHeJiMap.get("hujiaozhongxingCountPSum").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 9, fuwuzhanHeJiMap.get("xuexizhongxinCountSum").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 10, fuwuzhanHeJiMap.get("xuexizhongxinCountPSum").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 11, fuwuzhanHeJiMap.get("lishishujuCountSum").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 12, fuwuzhanHeJiMap.get("lishishujuCountPSum").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 13, fuwuzhanHeJiMap.get("hejiCountSum").toString(),HSSFColor.WHITE.index);
						createCell(wb, bodyRow, 14, fuwuzhanHeJiMap.get("hejiCountPSum").toString(),HSSFColor.WHITE.index);
			...
			createCell(wb, bodyRow, 5, result.get("wangluobaomingSum").toString(),HSSFColor.YELLOW.index);
			createCell(wb, bodyRow, 6, result.get("wangluobaomingSumP").toString(),HSSFColor.YELLOW.index);
			createCell(wb, bodyRow, 7, result.get("hujiaozhongxinSum").toString(),HSSFColor.YELLOW.index);
			createCell(wb, bodyRow, 8, result.get("hujiaozhongxinSumP").toString(),HSSFColor.YELLOW.index);
			createCell(wb, bodyRow, 9, result.get("xuexizhongxinSum").toString(),HSSFColor.YELLOW.index);
			createCell(wb, bodyRow, 10, result.get("xuexizhongxinSumP").toString(),HSSFColor.YELLOW.index);
			createCell(wb, bodyRow, 11,result.get("lishishujuSum").toString(),HSSFColor.YELLOW.index);
			createCell(wb, bodyRow, 12,result.get("lishishujuSumP").toString(),HSSFColor.YELLOW.index);
			createCell(wb, bodyRow, 13, result.get("heji").toString(),HSSFColor.YELLOW.index);
