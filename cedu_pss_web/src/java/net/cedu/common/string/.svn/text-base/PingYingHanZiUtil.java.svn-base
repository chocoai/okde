/**
 * 文件名：PingYingHanZiUtil.java
 * 包名：net.cedu.common.string
 * 工程：cedu_pss_web
 * 功能： TODO /请自行添加
 *
 * 作者：yangdongdong    
 * 日期：2012-1-7 下午12:47:15
 *
 */
 ...
 * 
 */
public  class PingYingHanZiUtil {

	//特殊情况字符串转换Map
	final static Map<String,String> strConvertMap = new HashMap<String, String>(){
		{
			put("1","一");
			put("2","二");
			put("3","三");
	...
	public static Vector<String> expand(String strTextPass) {
	...
	public static char getNameFirstZiMuToLowerCase(String nameStr){
		Vector<String> name = PingYingHanZiUtil.getLetter(nameStr);
		return name.get(0).toLowerCase().toCharArray()[0];
	}
	public static char getNameFirstZiMuToUpperCaseCase(String nameStr){
		Vector<String> name = PingYingHanZiUtil.getLetter(nameStr);
		return name.get(0).toUpperCase().toCharArray()[0];
	}
	
	public static String getPingYingNameToLower(String chineseName){
		Vector<String> ans = PingYingHanZiUtil.expand(convertString(chineseName));
		return ans.get(1).toLowerCase();
	}
	/*
	 * 重写getPingYingNameToLower方法，由于个别系统不识别的文字会出现头部为数字的问题
	 * 导致map起名的时候会出现BUG(起名不能以数字开头)为了不影响排序相关功能所以全部加一个“a”
	 */
	public static String getPingYingNameToLowerNew(String chineseName){
		Vector<String> ans = PingYingHanZiUtil.expand(convertString(chineseName));
		return "a"+ans.get(1).toLowerCase();
	}

	public  static void main(String[] args) {
//		System.out.println(getNameFirstZiMuToLowerCase("我"));
//		Vector<String> ans = PingYingHanZiUtil.expand("王小二");
//		System.out.println(ans.get(1).toLowerCase());
		convertString("备333dd用1的0");
	}
	...
	private static String convertString(String str){
		...
		for(char ch : chs){
			String s = strConvertMap.get(ch+"");
			strSb.append(s==null?ch:s);
		}
